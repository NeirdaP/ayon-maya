...
class RunPublishHooks(plugin.MayaInstancePlugin, OptionalPyblishPluginMixin):
    """
    Plugin to run arbitrary project-defined python scripts that modify/process the product
    """
    optional = True
    order = pyblish.api.ExtractorOrder + 0.49
    label = "Run Publish Hooks"
    families = ["rig"]

    def process(self, instance):        
        self.log.info("Retrieving hooks profiles from project settings")
        publish_hook_profiles = ayon_api.get_addon_project_settings(
                                        "maya", 
                                        version.__version__, 
                                        instance.context.data["projectName"]).get("publish").get("RunPublishHooks").get("profiles")
        self.log.debug(f"{publish_hook_profiles=}")

        publish_hooks_dir = None 
        ...
        for profile in publish_hook_profiles:
            if curr_task_type in profile.get("task_types"):
                publish_hooks_dir = profile.get("path")

        python_hooks = pathlib.Path(publish_hooks_dir).rglob("*.py") if publish_hooks_dir else None

        if not publish_hooks_dir or not python_hooks:
            self.log.warning("Hooks directory is not defined or is empty. Skipping plugin execution...")
            return
